...
output_dir = "./Outputs_Part_3"
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Create a directory int the output directory in which the Heatmap plots will be saved
plots_dir = "/Heatmap Plots"
if not os.path.exists(output_dir + plots_dir):
    os.makedirs(output_dir + plots_dir)

# Clear the content of the file if it already exists; Otherwise, create the file.
if os.path.exists(output_dir + '/Attention_Weights.json'):
    os.remove(output_dir + '/Attention_Weights.json')
...
def train(train_src, train_trg, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, src_dev_example,
          trg_dev_example, src_dev_example_txt, trg_dev_example_txt, epochs=10):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for idx, (src_seq, trg_seq) in enumerate(zip(src, trg)):
            ...
            # Initialize the encoder hidden vector and the outputs matrix
        ...
        analyze_attention_weights(src_dev_example, trg_dev_example, src_dev_example_txt, trg_dev_example_txt, encoder,
                                  decoder, epoch)

        print("Epoch: {}/{}...".format(epoch, epochs), "Train Loss: {:.3f}...".format(train_loss))

    passed_time = time.time() - start_time
    print('The training ended after %.3f minutes' % (passed_time / 60))


def analyze_attention_weights(src_dev_example, trg_dev_example, src_dev_example_txt, trg_dev_example_txt, encoder,
                              decoder, epoch):

    with torch.no_grad():

        input_length = src_dev_example.size(0)
        target_length = trg_dev_example.size(0)
        ...
        for i in range(input_length):
            ...
            # Forward pass via the Encoder
            encoder_output, encoder_hidden = encoder(src_dev_example[i], encoder_hidden)
        ...
        decoder_attentions = []
        decoder_input = trg_dev_example[0]
        ...
        # Plot heatmap of the attention  weights
        plot_attention("/Epoch_" + str(epoch), src_dev_example_txt[1:input_length - 1],
                       trg_dev_example_txt[1:input_length - 1], torch.cat(decoder_attentions, dim=0))

        # Dumps the attention weights for a specific example from the development set
        with open(output_dir + '/Attention_Weights.json', 'a+', encoding='utf-8') as f:
            attentions = [attention.tolist()[0] for attention in decoder_attentions]
            f.write("Epoch {}:\n".format(epoch))
            for attention in attentions:
                json.dump(attention, f)
                f.write("\n")
            f.write("\n")


def plot_attention(name_plot, input_sentence, output_words, attentions):

    fig, ax = plt.subplots(1, figsize=(8, 8), dpi=192)
    fig.subplots_adjust(left=0, right=1, bottom=0, top=1)

    ax.set_xticks(np.arange(len(input_sentence) + 2))
    ax.set_yticks(np.arange(len(input_sentence) + 1))

    ax = sns.heatmap(attentions.numpy(), square=True, cmap="Blues", xticklabels=['begin'] + input_sentence + ['end'],
                     annot=True, cbar=False)

    ax.set_yticklabels(output_words + ['end'], rotation=360)

    ax.set(title="Attention-based Alignment:\n{0} -> {1}\n".format(' '.join(input_sentence), ' '.join(output_words)),
           xlabel="Input",
           ylabel="Output")

    fig.tight_layout()
    plt.savefig(output_dir + plots_dir + name_plot + ".png", dpi=192, bbox_inches="tight")
...
def main():
    ...
    # Loading the dev set
    dev_data, dev_src, dev_trg = read_data(SRC_DEV, TRG_DEV)

    dev_src_text = update_with_unk(dev_src, s2i)
    ...
    # Training
    train(train_src_tensors, train_trg_tensors, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,
          dev_src_tensors[CHOSEN_EXAMPLE], dev_trg_tensors[CHOSEN_EXAMPLE], dev_src_text[CHOSEN_EXAMPLE],
          dev_trg_text[CHOSEN_EXAMPLE])
